...
-- カット4 ----------------------------------------------------------------------------------------
	...
	open_mind_window("CHRNAME_NOIR", Noir, nil)
	...
	keep_delay_ik_lookat(Noir, Lancelot, "J_Head", 0.5)
	
	open_mind_window("CHRNAME_NOIR", Noir, nil)
	message_direct("keep_delay_ik_lookat(Noir, Dinatan, J_Head, 1.5) 旋回速度1.5倍速")
	close_mind_window()
	
	keep_delay_ik_lookat(Noir, Dinatan, "J_Head",1.5)
	
	open_mind_window("CHRNAME_NOIR", Noir, nil)
	message_direct("lookat_delay_weight_reset(Noir ,1) weightをリセットする、等速で正面に向く")
	close_mind_window()
	
	lookat_delay_weight_reset(Noir , 1)
	
	open_mind_window("NPCNAME_LANCELOT", Lancelot, nil)
	message_direct("keep_ik_lookat_object(Lancelot, Bgarea_103_doorG, geo_door_R) 親オブジェを指定して注視したいオブジェを設定する")
	close_mind_window()
	
	keep_ik_lookat_object(Lancelot, "Bgarea_103_doorG", "geo_door_R")
	
	open_mind_window("CHRNAME_NOIR", Noir, nil)
	message_direct("keep_ik_lookat_object(Noir, nil, geo_chair_01) lookat_delay_weight_default(Noir, 1)親オブジェはnilでも可能 旋回速度等速")
	close_mind_window()
	
	keep_ik_lookat_object(Noir, nil, "geo_chair_01")
	lookat_delay_weight_default(Noir, 1)
	
	open_mind_window("CHRNAME_NOIR", Noir, nil)
	message_direct("keep_delay_ik_lookat_object(Noir, nil, geo_door_R , 1)親オブジェはnilでも可能 旋回速度等速")
	close_mind_window()
	
	keep_delay_ik_lookat_object(Noir, nil, "geo_door_R" , 1)
	
	local obj = create_object("kari")
	local obj2 = create_object("kari2")
	
	set_pos_object(obj, -7 , -10 , 2)
	set_pos_object(obj2, 2 , 10 , 0)
	
	open_mind_window("NPCNAME_LANCELOT", Lancelot, nil)
	message_direct("keep_ik_lookat_object(Lancelot, nil, kari ) オブジェクトを生成して注視する(下方")
	close_mind_window()
	
	keep_ik_lookat_object(Lancelot, nil, "kari" )
	
	open_mind_window("NPCNAME_LANCELOT", Lancelot, nil)
	message_direct("keep_ik_lookat_object(Lancelot, nil, kari2 ) オブジェクトを生成して注視する（上方")
	close_mind_window()
	
	keep_ik_lookat_object(Lancelot, nil, "kari2" )
	
	open_mind_window("CHRNAME_NOIR", Noir, nil)
	message_direct("keep_ik_lookat_object(Noir, nil, kari ) オブジェクトを生成して注視する(下方 旋回速度等速")
	close_mind_window()
	
	keep_delay_ik_lookat_object(Noir, nil, "kari" , 1)
	
	open_mind_window("CHRNAME_NOIR", Noir, nil)
	message_direct("keep_ik_lookat_object(Noir, nil, kari2 , 0.5) オブジェクトを生成して注視する(上方 旋回速度半分")
	close_mind_window()
	
	keep_delay_ik_lookat_object(Noir, nil, "kari2" , 0.5)
	
	open_mind_window("CHRNAME_NOIR", Noir, nil)
	message_direct("keep_delay_ik_lookat_object(Noir, nil, kari, 1) オブジェクトを生成して注視するがoffset設定で座標を打ち消して0,0,0を見る")
	close_mind_window()
	
	set_lookat_offset(Noir, 7,10,-2)
	keep_delay_ik_lookat_object(Noir, nil, "kari" , 1)
	
	open_mind_window("CHRNAME_NOIR", Noir, nil)
	message_direct("keep_delay_ik_lookat_object(Noir, nil, kari2, 1) オブジェクトを生成して注視するがoffset設定で座標を打ち消して0,0,0を見る")
	close_mind_window()
	
	local offset = {-2,-10,0}
	set_lookat_offset(Noir, offset)
	keep_delay_ik_lookat_object(Noir, nil, "kari2" , 1)
	
	open_mind_window("NPCNAME_LANCELOT", Lancelot, nil)
	message_direct("owari")
	close_mind_window()
