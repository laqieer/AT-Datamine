Include("content_luascript_system", "LuaCommonPack")
Include("content_luascript_system", "PsBattleAdv")		-- 疑似バトルADVの時はIncludeする

-- アセットなどを読み込む
function Load()
	Load_common(true)
	
	-- 初期化とマップ読み込みを行う
	init_psbattleadv("201000000", "201000000")
	
	-- プレイヤーキャラ予約
	dotnoir = dot_reserve_ally(101009001, 16, 16, 0)
	
	-- プレイヤーキャラの武器データは必ずセットしなければならない
	dot_set_weapon(dotnoir, 101010001)		-- ロングソード
	dot_set_weapon(dotnoir, 105010001)		-- ウッドボウ
	
	-- ティルフィング
	dottyr = dot_reserve_ally(101001002, 16, 17, 0)
	dot_set_weapon(dottyr, 101010005)		-- キラーブレイド
	--dot_set_weapon(dottyr, 107010004)		-- 「禁書」金獅子
	
	
	-- 敵配置予約
	barbaroi = dot_reserve_enemy(3, 18, 17, 0)
	
	griffin = dot_reserve_enemy(100033, 18, 18, 0)
	
	enemy3 = dot_reserve_enemy(27, 14, 15, 0)
	
	--enemy4 = dot_reserve_enemy(57, 15, 16, 0)
	
	--[[
	for i = 1, 5 do
		for j = 1 , 5 do
			dot_reserve_enemy(1, i+15, j+15, 0)
		end
	end
	]]
	
	-- 実際にユニットを配置する。これ以降ユニットを操作する事ができるようになる。
	-- 予約したユニット情報に基づいて、ユニット配置やパーティクル読み込みが行われる。
	setup_complete_unit()
	
	dot_unit_dir(barbaroi, DotDir_Left)
	dot_change_anim(barbaroi, DotAnim_Squat)
	
	--dot_disp_weapon(barbaroi, false)
	
	--dot_disp_unit(enemy3, false)
	
	-- カメラテスト
	change_camera_distance(10.0, 0.0)
	change_camera_angle(25.0, 0.0)
	move_camera(16, 15, 0.0)
end



function select_duration()
	open_select_window_direct(nil, "Normal", "0秒", "1秒", "3秒", "5秒", "10秒")
	if is_select(1) then
		return 0.0
	elseif is_select(2) then
		return 1.0
	elseif is_select(3) then
		return 3.0
	elseif is_select(4) then
		return 5.0
	elseif is_select(5) then
		return 10.0
	end
end


-- カメラテスト
function camera_test()
	
	local x = 16
	local y = 15
	local moveflag = false
	local angle = 25.0
	local angleflag = false
	local distance = 10.0
	local distanceflag = false
	local waitflag = true
	local duration
	
	open_narration_window()
	message_direct("動作種類と時間を選択してください")
	close_narration_window()

	while true do
		open_select_window_direct(nil, "Normal", "グリッド移動", "角度変更", "距離変更", "動作待ち変更", "戻る")
		if is_select(1) then
			duration = select_duration()
			if moveflag == false then
				move_camera(x + 10, y + 10, duration)
				moveflag = true
			else
				move_camera(x, y, duration)
				moveflag = false
			end
		elseif is_select(2) then
			duration = select_duration()
			if angleflag == false then
				change_camera_angle(90.0, duration)
				angleflag = true
			else
				change_camera_angle(angle, duration)
				angleflag = false
			end
		elseif is_select(3) then
			duration = select_duration()
			if distanceflag == false then
				change_camera_distance(30.0, duration)
				distanceflag = true
			else
				change_camera_distance(distance, duration)
				distanceflag = false
			end
		elseif is_select(4) then
			if waitflag == true then
				waitflag = false
				open_narration_window()
				message_direct("カメラの動作待ちしません")
				close_narration_window()
			else
				waitflag = true
				open_narration_window()
				message_direct("カメラの動作待ちします")
				close_narration_window()
			end
		else
			break
		end
		if waitflag == true then
			-- カメラ動作終了待ち処理
			wait_camera()
		end
	end
	
end

-- キャラクター選択
function select_unit()
	open_narration_window()
	message_direct("対象のキャラクターを選んで下さい")
	close_narration_window()
	open_select_window_direct(nil, "Normal", "ノワール", "ティルフィング", "バルバロイ", "グリフォン", "戻る")
	
	if is_select(1) then
		return dotnoir
	elseif is_select(2) then
		return dottyr
	elseif is_select(3) then
		return barbaroi
	elseif is_select(4) then
		return griffin
	else
		return nil
	end
	
end


function unit_test()
	while true do
		dotchar = select_unit()
		if dotchar == nil then
			break
		end
		
		open_narration_window()
		message_direct("移動先を選んで下さい")
		close_narration_window()
		
		open_select_window_direct(nil, "Normal", "X16 Y16", "X15 Y18 ", "X15 Y20", "X15 Y21", "戻る")
		if is_select(1) then
			dot_move_unit_seq(dotchar, 16, 16)
		elseif is_select(2) then
			dot_move_unit_seq(dotchar, 15, 18)
		elseif is_select(3) then 
			dot_move_unit_seq(dotchar, 15, 20)
		elseif is_select(4) then
			dot_move_unit_seq(dotchar, 15, 21)
		else
			break
		end
		wait_seq()
	end
end


-- シーケンスの実行
function Play()
	fadein(0)
	Play_start(true)
	
	open_narration_window()
	message_direct("疑似バトルADVテスト")
	close_narration_window()
	
	dot_dying_unit_seq(enemy3)
	
	while true do
		open_select_window_direct(nil, "Normal", "カメラテスト", "ユニットテスト", "終了")
		if is_select(1) then
			camera_test()
		elseif is_select(2) then
			unit_test()
		else
			break
		end
	end
	
	open_narration_window()
	message_direct("終了します")
	close_narration_window()
	
	Play_end(true)
end


function PreLoad()
	PreLoad_AutoGenerate()
	
	-- 手動追加時はここの下に追加してください。
end

function PreLoad_AutoGenerate()
--luaのインポート時にPreLoadする対象をLoadから自動検出して追加する
end
