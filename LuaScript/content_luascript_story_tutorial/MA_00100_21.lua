-- このluaスクリプトは、MA_00100_21.csvから自動生成されました --
...
-- ▲直接出力
	...
	--★★アルハン_01★★:見下すな、仰ぎ見ろ<br>ワタシに耳をかたむけよ
	Talk(Actor001,"NPCNAME_0312","speech","L","MA_00100_210002")
...
-- ▲直接出力
-- ▼直接出力
--アルハン_01,NPCNAME_0313 @名前変更
-- ▲直接出力

	--★★アルハン_01★★:ワタシはアルハン_01<br>継承者を導く者
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_210004")

-- ▼直接出力
setup_small_camera_end(Camera002)
--set_animationcontroller(Actor001, "Npc_1001_01_Controller","to Std_Loop")
SkipOffsetCamera(Camera002)
-- ▲直接出力
	change_face(Actor002,"Sad")

	--★★Player★★:…？
	Talk(Actor002,"PLAYERNAME_GENDAI","speech","L","MA_00100_210006")

-- ▼直接出力
setup_small_camera_start()
-- ▲直接出力

	--★★アルハン_01★★:まもなく世界は終わり<br>歴史は幕を閉じる
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_210008")


	--★★アルハン_01★★:届いていたな？この声が
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_210012")


	--★★アルハン_01★★:たったひとつの救いの道<br>そこにキサマは踏み入れた
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_210014")
...
-- ▲直接出力
	...
	--★★アルハン_01★★:人類の歴史は<br><ruby=バルバロイ>侵略者</ruby>との戦いの歴史
	Talk(Actor001,"NPCNAME_0313","simple","N","MA_00100_220002")
...
-- ▲直接出力
	...
	--★★アルハン_01★★:歴史の節目節目に来襲するバルバロイ<br>人間たちはその都度武器を取り、妨げ続けてきた
	Talk(Actor001,"NPCNAME_0313","simple","N","MA_00100_220004")
...
-- ▲直接出力
	...
	--★★アルハン_01★★:だがそれも限界だ<br>未曾有の脅威が迫っている
	Talk(Actor001,"NPCNAME_0313","simple","N","MA_00100_220006")


	--★★アルハン_01★★:これを阻めるのは<br>世界においてただひとり
	Talk(Actor001,"NPCNAME_0313","simple","N","MA_00100_220008")
...
-- ▲直接出力
	...
	--★★アルハン_01★★:お前だ<br><dot>最後の継承者</dot><%player>
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_230002")

-- ▼直接出力
DontChangeRandomCamera(false)
-- ▲直接出力
	open_select_window_tag(Actor002,"Normal","MA_00100_230003","MA_00100_230004")
	if is_select(1) then
		goto Block2_1
...
-- ▲直接出力
	...
	--★★アルハン_01★★:わからないのか？<br>本当に？
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_230006")

	change_face(Actor001,"Normal")

	--★★アルハン_01★★:キサマに可能性があるからだ
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_230008")

	change_face(Actor001,"Anger")

	--★★アルハン_01★★:キサマがここまでやってきたからだ
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_230010")
...
-- ▲直接出力
	...
	--★★アルハン_01★★:そう。最後だ
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_230012")

	change_face(Actor001,"Normal")

	--★★アルハン_01★★:キサマのあとに<br>歴史は続かない
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_230014")

	change_face(Actor001,"Smile")

	--★★アルハン_01★★:世界がどうなろうとな
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_230016")
...
-- ▲直接出力
	...
	--★★アルハン_01★★:過去、バルバロイを討ったのは伝説の武器たちだ<br>それを現在まで継承してくれた者たちがいる
	Talk(Actor001,"NPCNAME_0313","simple","N","MA_00100_230018")
...
-- ▲直接出力
	...
	--★★アルハン_01★★:だがその武器は時の流れにより風化し<br>今や元来の力を発揮できる状態ではない
	Talk(Actor001,"NPCNAME_0313","simple","N","MA_00100_230020")
...
-- ▲直接出力
	...
	--★★アルハン_01★★:終末を回避するためには<br>武器の歴史へと潜り…
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_230022")
...
-- ▲直接出力
	...
	--★★アルハン_01★★:それらの記憶を呼び覚ます必要がある
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_230024")
...
-- ▲直接出力
	open_select_window_tag(Actor002,"Normal","MA_00100_230025","MA_00100_230026","MA_00100_230027")
	if is_select(1) then
		goto Block3_1
	elseif is_select(2) then
		goto Block3_2
...
-- ▲直接出力
	...
	--★★アルハン_01★★:武器に秘められた歴史の可能性を<br>キサマが体験し、一本に<dot>選び抜く</dot>のだ
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_230029")


	--★★アルハン_01★★:さすれば現在へと蘇る…<br>キサマが選んだ唯一無二の伝説が
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_230033")


	--★★アルハン_01★★:その<ruby=レアリティ>稀少性</ruby>が力となる
	Talk(Actor001,"NPCNAME_0313","speech","L","MA_00100_230035")
